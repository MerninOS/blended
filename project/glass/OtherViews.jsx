        ...
        <div style={{display:"flex",gap:6}}>
          {[["all","All"],["pro","Pro"],["standard","Standard"],["new","New"],["overdue","Overdue"]].map(([k,l])=>(
            <button key={k} onClick={()=>setFilter(k)} style={{
              padding:"5px 11px",borderRadius:11999,border:"1px solid rgba(60,40,28,0.10)",
              background:filter===k?"var(--color-espresso)":"var(--color-cream)",
              color:filter===k?"var(--color-cream)":"var(--color-espresso)",
              fontWeight:800,fontSize:10.5,letterSpacing:".09em",textTransform:"uppercase",cursor:"pointer",
            }}>{l}</button>
          ))}
        </div>
      }>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
          <thead><tr style={{borderBottom:"1px solid rgba(60,40,28,0.08)"}}>
            {["Roaster","Tier","Since","Lbs · MTD","Owed","Status",""].map((h,i)=>(
              <th key={i} style={{textAlign: i>=3&&i<=4 ? "right" : "left", padding:"8px 10px", fontSize:10, letterSpacing:".11em", textTransform:"uppercase", color:"var(--fg2)", fontWeight:800}}>{h}</th>
            ))}
          </tr></thead>
          <tbody>
            {filtered.map(c=>(
  ...
  return (
    <div style={{padding:"16px 24px 32px",display:"flex",flexDirection:"column",gap:14}}>
      ...
      <Panel title="Shelf Map" actions={<div style={{display:"flex",gap:10,fontSize:11,fontWeight:700}}>
        <span style={{display:"inline-flex",alignItems:"center",gap:5}}><span style={{width:12,height:12,background:"var(--color-matcha)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:3}}/>Active</span>
        <span style={{display:"inline-flex",alignItems:"center",gap:5}}><span style={{width:12,height:12,background:"var(--color-tomato)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:3}}/>Past due</span>
        <span style={{display:"inline-flex",alignItems:"center",gap:5}}><span style={{width:12,height:12,background:"rgba(255,253,250,0.65)",backdropFilter:"blur(16px) saturate(140%)",WebkitBackdropFilter:"blur(16px) saturate(140%)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:3}}/>Open</span>
      </div>}>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {rows.map(r=>(
            <div key={r} style={{display:"flex",alignItems:"stretch",gap:10}}>
              <div style={{width:54,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(36,24,15,0.72)",backdropFilter:"blur(24px) saturate(140%)",WebkitBackdropFilter:"blur(24px) saturate(140%)",color:"var(--color-cream)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:12,fontFamily:"var(--font-display)",fontSize:30,lineHeight:1}}>{r}</div>
              <div style={{flex:1,display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(180px, 1fr))",gap:8}}>
                {SHELVES.filter(s=>s.row===r).map(s=>{
                  ...
                  return (
                    <div key={s.id} style={{padding:10,border:"1px solid rgba(60,40,28,0.10)",borderRadius:12,background:bg,boxShadow:"0 2px 8px rgba(28,15,5,0.05)",display:"flex",flexDirection:"column",gap:6,minHeight:88}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                        <span style={{fontFamily:"var(--font-mono)",fontWeight:800,fontSize:11,padding:"2px 6px",background:"rgba(36,24,15,0.72)",backdropFilter:"blur(24px) saturate(140%)",WebkitBackdropFilter:"blur(24px) saturate(140%)",color:"var(--color-cream)",borderRadius:5}}>{s.row}-{s.num}</span>
                      ...
                      {c ? <>
                        ...
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:10,fontWeight:700,color:"var(--fg2)",letterSpacing:".05em",marginTop:"auto"}}>
                          <span>${s.rate}/mo</span>
                          {s.status==="overdue" ? <span style={{color:"var(--color-tomato)",fontWeight:800,textTransform:"uppercase"}}>Past Due</span> : <span>Since {s.since.slice(0,7)}</span>}
                        </div>
                      </> : <>
                        <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,letterSpacing:".1em",textTransform:"uppercase",color:"var(--fg2)"}}>Open</div>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
          <thead><tr style={{borderBottom:"1px solid rgba(60,40,28,0.08)"}}>
            {["Shelf","Roaster","Size","Renting Since","Rate","Status",""].map((h,i)=>(
              <th key={i} style={{textAlign:i===4?"right":"left",padding:"8px 10px",fontSize:10,letterSpacing:".11em",textTransform:"uppercase",color:"var(--fg2)",fontWeight:800}}>{h}</th>
            ))}
          </tr></thead>
          <tbody>
            {active.map(s=>{
              ...
              return (
                <tr key={s.id} style={{borderBottom:"1px dashed var(--color-fog)"}}>
                  <td style={{padding:"9px 10px"}}><span style={{fontFamily:"var(--font-mono)",fontWeight:800,fontSize:12,padding:"3px 8px",background:"rgba(36,24,15,0.72)",backdropFilter:"blur(24px) saturate(140%)",WebkitBackdropFilter:"blur(24px) saturate(140%)",color:"var(--color-cream)",borderRadius:5}}>{s.row}-{s.num}</span></td>
  ...
  <div style={{padding:"16px 24px 32px",display:"flex",flexDirection:"column",gap:14}}>
    <Panel title="What We Offer" actions={<Btn size="sm" variant="primary" icon={<I name="plus" size={14}/>}>Add Service</Btn>}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(310px, 1fr))",gap:12}}>
        {SERVICES.map(s=>(
          <div key={s.id} style={{padding:14,border:"1px solid rgba(60,40,28,0.10)",borderRadius:18,background:"rgba(255,255,255,0.72)",backdropFilter:"blur(20px) saturate(160%)",WebkitBackdropFilter:"blur(20px) saturate(160%)",boxShadow:"0 4px 14px rgba(28,15,5,0.06), 0 1px 2px rgba(28,15,5,0.03)",display:"flex",flexDirection:"column",gap:8}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
              <div style={{fontFamily:"var(--font-display)",fontSize:22,lineHeight:1,textTransform:"uppercase"}}>{s.label}</div>
            ...
            <div style={{display:"flex",alignItems:"baseline",gap:8,marginTop:4}}>
              ...
              <div style={{fontSize:11,fontWeight:800,letterSpacing:".09em",textTransform:"uppercase",color:"var(--fg2)"}}>per {s.unit}</div>
            </div>
...
const RuleRow = ({label,value,hint}) => (
  <div style={{padding:12,border:"1px solid rgba(60,40,28,0.10)",borderRadius:14,background:"rgba(255,253,250,0.65)",backdropFilter:"blur(16px) saturate(140%)",WebkitBackdropFilter:"blur(16px) saturate(140%)"}}>
    <div style={{fontSize:10,fontWeight:800,letterSpacing:".11em",textTransform:"uppercase",color:"var(--fg2)"}}>{label}</div>
    <div style={{fontFamily:"var(--font-display)",fontSize:22,textTransform:"uppercase",lineHeight:1,marginTop:5}}>{value}</div>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{borderBottom:"1px solid rgba(60,40,28,0.08)"}}>
              {["Invoice","Roaster","Issued","Due","Sync","Amount",""].map((h,i)=>(
                <th key={i} style={{textAlign:i>=5&&i<=5?"right":"left",padding:"7px 10px",fontSize:10,letterSpacing:".11em",textTransform:"uppercase",color:"var(--fg2)",fontWeight:800}}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {g.items.map(inv=>{
...
const IntegrationsBar = ({draftCount}) => (
  <div style={{padding:"12px 16px",background:"rgba(36,24,15,0.72)",backdropFilter:"blur(24px) saturate(140%)",WebkitBackdropFilter:"blur(24px) saturate(140%)",color:"var(--color-cream)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:18,boxShadow:"0 8px 24px rgba(184,98,58,0.18), 0 1px 3px rgba(28,15,5,0.05)",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",flex:1,minWidth:0}}>
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",background:"rgba(255,255,255,.08)",borderRadius:10,whiteSpace:"nowrap"}}>
        <QBLogo size={14}/>
        <div style={{lineHeight:1.15,whiteSpace:"nowrap"}}>
          ...
          <div style={{fontSize:9.5,opacity:.7,letterSpacing:".06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>Synced 14m ago</div>
        </div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",background:"rgba(255,255,255,.08)",borderRadius:10,whiteSpace:"nowrap"}}>
        ...
        <div style={{lineHeight:1.15,whiteSpace:"nowrap"}}>
          ...
          <div style={{fontSize:9.5,opacity:.7,letterSpacing:".06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>Live · acct_1Q…7sN</div>
        </div>
        <span style={{width:8,height:8,borderRadius:11999,background:"var(--color-matcha)",marginLeft:4,flexShrink:0}}/>
      </div>
    </div>
    {draftCount>0 && (
      <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        <div style={{fontSize:11.5,fontWeight:700,textAlign:"right",lineHeight:1.25,whiteSpace:"nowrap"}}>
          ...
          <div style={{opacity:.65,fontSize:10,letterSpacing:".06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>From completed sessions</div>
        </div>
        <button style={{padding:"8px 14px",border:"1px solid rgba(255,255,255,0.6)",background:"rgba(255,253,250,0.65)",backdropFilter:"blur(16px) saturate(140%)",WebkitBackdropFilter:"blur(16px) saturate(140%)",color:"var(--color-espresso)",borderRadius:11,fontWeight:800,fontSize:12,fontFamily:"var(--font-body)",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:6,letterSpacing:".02em",whiteSpace:"nowrap"}}>
