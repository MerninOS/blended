...
const ScheduleView = ({ tweaks, openBooking, dayKey, setDayKey }) => {
  ...
  const blockStyle = (b, res) => {
    ...
    return {
      ...
      borderRadius:12,
      boxShadow: b.status==="inprogress" ? "3px 3px 0 var(--color-espresso), 0 0 0 3px var(--color-sun)" : "3px 3px 0 var(--color-espresso)",
    ...
    <div style={{padding:"16px 24px 32px",display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{fontFamily:"var(--font-display)",fontSize:26,textTransform:"uppercase",lineHeight:1,minWidth:280}}>{dayLabel}</div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          {Object.entries(BOOKING_TYPES).filter(([k])=>k!=="storage").map(([k,m])=>(
            <span key={k} style={{display:"inline-flex",alignItems:"center",gap:6,fontSize:11,fontWeight:700,letterSpacing:".05em"}}>
              <span style={{width:14,height:14,background:`var(--color-${m.color})`,border:"1px solid rgba(60,40,28,0.10)",borderRadius:4}}/>
              {m.label}
            </span>
          ))}
      ...
      <Panel noPadding>
        <div style={{display:"flex",fontFamily:"var(--font-body)",position:"relative"}}>
          ...
          <div style={{flexShrink:0,width:LABEL_W,borderRight:"1px solid rgba(60,40,28,0.08)",background:"rgba(255,253,250,0.65)",backdropFilter:"blur(16px) saturate(140%)",WebkitBackdropFilter:"blur(16px) saturate(140%)"}}>
            {/* header spacer */}
            <div style={{height:36,borderBottom:"1px solid rgba(60,40,28,0.08)",display:"flex",alignItems:"center",padding:"0 14px",fontSize:10,fontWeight:800,letterSpacing:".12em",textTransform:"uppercase",color:"var(--fg2)",background:"rgba(255,255,255,0.72)",backdropFilter:"blur(20px) saturate(160%)",WebkitBackdropFilter:"blur(20px) saturate(160%)"}}>Resource</div>
            {RESOURCES.map(r=>(
              <div key={r.id} style={{height:ROW_H,borderBottom:"1px solid var(--color-fog)",padding:"0 14px",display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:32,height:32,background:`var(--color-${r.color})`,border:"1px solid rgba(60,40,28,0.10)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",color: (r.color==="sun"||r.color==="sky"||r.color==="cream") ? "var(--color-espresso)" : "var(--color-cream)",flexShrink:0}}>
                ...
                <div style={{minWidth:0}}>
                  ...
                  <div style={{fontSize:10,color:"var(--fg2)",letterSpacing:".06em",textTransform:"uppercase",fontWeight:700,marginTop:2}}>{r.capacity} · ${r.rate}/hr</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{flex:1,overflowX:"auto",position:"relative"}}>
            <div style={{position:"relative",width:totalSlots*SLOT_W,minWidth:"100%"}}>
              {/* Header: hours */}
              <div style={{display:"flex",height:36,borderBottom:"1px solid rgba(60,40,28,0.08)",background:"rgba(255,255,255,0.72)",backdropFilter:"blur(20px) saturate(160%)",WebkitBackdropFilter:"blur(20px) saturate(160%)"}}>
                  ...
                  {dayBookings.filter(b=>b.resource===r.id).map(b=>{
                    ...
                    return (
                      <button key={b.id} onClick={()=>openBooking(b)} style={blockStyle(b,r)}
                        onMouseOver={e=>{e.currentTarget.style.transform="translate(-1.5px,-1.5px)";e.currentTarget.style.boxShadow="5px 5px 0 var(--color-espresso)"+(b.status==="inprogress"?", 0 0 0 3px var(--color-sun)":"")}}
                        onMouseOut={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="3px 3px 0 var(--color-espresso)"+(b.status==="inprogress"?", 0 0 0 3px var(--color-sun)":"")}}>
                        <div style={{display:"flex",alignItems:"center",gap:6,minWidth:0}}>
                          <span style={{fontSize:9,fontWeight:900,letterSpacing:".12em",textTransform:"uppercase",padding:"1px 5px",background:"rgba(28,15,5,0.18)",borderRadius:4}}>{meta.label}</span>
                          {b.status==="inprogress" && <Dot color="cream" pulse size={7}/>}
                          {b.recurring && <I2 name="repeat" size={11}/>}
                          {!b.paid && b.status!=="completed" && <span title="unpaid" style={{fontSize:9,fontWeight:900,letterSpacing:".08em",textTransform:"uppercase",background:"rgba(36,24,15,0.72)",backdropFilter:"blur(24px) saturate(140%)",WebkitBackdropFilter:"blur(24px) saturate(140%)",color:"var(--color-cream)",padding:"1px 4px",borderRadius:3}}>$</span>}
                        </div>
                        <div style={{minWidth:0}}>
              ...
              {showNow && (
                <div style={{
                  ...
                  <div style={{position:"absolute",top:-6,left:-7,width:16,height:16,background:"var(--color-tomato)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:11999}}/>
                  <div style={{position:"absolute",top:-22,left:6,fontSize:10,fontWeight:800,letterSpacing:".12em",textTransform:"uppercase",color:"var(--color-tomato)",fontFamily:"var(--font-body)",background:"rgba(255,253,250,0.65)",backdropFilter:"blur(16px) saturate(140%)",WebkitBackdropFilter:"blur(16px) saturate(140%)",padding:"1px 4px",border:"1.5px solid var(--color-tomato)",borderRadius:4,whiteSpace:"nowrap"}}>Now {fmtTime(NOW_HR)}</div>
                </div>
              )}
            </div>
          </div>
...
const navArrow = (disabled) => ({
  width:36,height:36,border:"1px solid rgba(60,40,28,0.10)",borderRadius:11999,
  background:disabled?"var(--color-fog)":"var(--color-cream)",cursor:disabled?"default":"pointer",
  display:"inline-flex",alignItems:"center",justifyContent:"center",
  boxShadow:disabled?"none":"3px 3px 0 var(--color-espresso)",
