              ...
              return (
                <button key={b.id} onClick={()=>openBooking(b)} style={floorCardStyle()}
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <div style={{width:48,height:48,background:"var(--color-tomato)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--color-cream)",position:"relative"}}>
                      <I name="flame" size={22}/>
                      <span style={{position:"absolute",top:-6,right:-6,width:14,height:14,background:"var(--color-sun)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:11999,animation:"co-pulse 1.5s ease-in-out infinite"}}/>
                    </div>
                  ...
                  <div style={{height:8,background:"var(--color-fog)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:11999,marginTop:10,overflow:"hidden",position:"relative"}}>
                    <div style={{position:"absolute",inset:0,width:pct+"%",background:"var(--color-tomato)",borderRight:"1px solid rgba(60,40,28,0.08)"}}/>
                  </div>
                </button>
              );
            })}
              ...
              return (
                <div key={k} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
                  <div style={{position:"relative",width:"100%",height:110,display:"flex",alignItems:"flex-end"}}>
                    <div style={{width:"100%",height:pct*1.05+"%",background:`var(--color-${meta.color})`,border:"1px solid rgba(60,40,28,0.10)",borderRadius:"8px 8px 0 0",boxShadow:"0 2px 8px rgba(28,15,5,0.05)",position:"relative"}}>
...
const AlertRow = ({icon,color,title,sub,cta,onClick}) => (
  <div style={{padding:12,border:"1px solid rgba(60,40,28,0.10)",borderRadius:14,background:"rgba(255,253,250,0.65)",backdropFilter:"blur(16px) saturate(140%)",WebkitBackdropFilter:"blur(16px) saturate(140%)",display:"flex",alignItems:"center",gap:12}}>
    <div style={{width:36,height:36,background:`var(--color-${color})`,border:"1px solid rgba(60,40,28,0.10)",borderRadius:11999,display:"flex",alignItems:"center",justifyContent:"center",color:(color==="sun"||color==="sky")?"var(--color-espresso)":"var(--color-cream)",flexShrink:0}}>
...
const floorCardStyle = () => ({
  padding:14,border:"1px solid rgba(60,40,28,0.10)",borderRadius:18,
  background:"rgba(255,255,255,0.72)",backdropFilter:"blur(20px) saturate(160%)",WebkitBackdropFilter:"blur(20px) saturate(160%)",boxShadow:"0 4px 14px rgba(28,15,5,0.06), 0 1px 2px rgba(28,15,5,0.03)",
  cursor:"pointer",transition:"all .12s var(--ease-snap)",fontFamily:"var(--font-body)",
  textAlign:"left",width:"100%",display:"block",
});

const upNextStyle = () => ({
  padding:"10px 12px",border:"1px solid rgba(60,40,28,0.10)",borderRadius:12,
  background:"rgba(255,253,250,0.65)",backdropFilter:"blur(16px) saturate(140%)",WebkitBackdropFilter:"blur(16px) saturate(140%)",boxShadow:"0 2px 8px rgba(28,15,5,0.05)",
  cursor:"pointer",transition:"all .12s var(--ease-snap)",fontFamily:"var(--font-body)",
...
const SignoffBanner = ({sessions, openBooking}) => {
  const NOW_HR = 10.3;
  return (
    <div style={{padding:"14px 18px",background:"var(--color-sun)",color:"var(--color-espresso)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:18,boxShadow:"0 4px 14px rgba(28,15,5,0.06), 0 1px 2px rgba(28,15,5,0.03)",display:"flex",alignItems:"stretch",gap:16}}>
      <div style={{width:48,height:48,background:"var(--color-espresso)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--color-sun)",flexShrink:0,alignSelf:"center"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {sessions.map(b=>{
            ...
            return (
              <div key={b.id} style={{padding:"9px 12px",background:"rgba(255,253,250,0.65)",backdropFilter:"blur(16px) saturate(140%)",WebkitBackdropFilter:"blur(16px) saturate(140%)",border:"1px solid rgba(60,40,28,0.10)",borderRadius:12,display:"flex",alignItems:"center",gap:12}}>
