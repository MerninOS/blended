// Roaster Portal × CoffeeOS — Blend builder: ratio worksheet + live tasting wheel.
// Ported from the CoffeeOS Blend Builder design into the portal's vocabulary.

// sections controls which blocks render and in what order, so a page can spread
// the builder across its own numbered steps: "add" | "ratios" | "roast" | "name"
const BlendRatios = ({ sel, setSel, roast, setRoast, blendName, setBlendName, retail, batchG = 5*PL_G_PER_LB, batchLabel,
  ...
  const minPct = (id) => plMinPctFor(id, batchG);
  // batch size can change under a finished blend (bag size, quantity, run lb)
  React.useEffect(()=>{
    if (!sel.length) return;
    if (!plMinsFit(sel, batchG)) return;
    if (sel.every(s=>s.pct >= plMinPctFor(s.id, batchG))) return;
    setSel(plEnforceMins(sel, null, batchG));
  }, [batchG, sel]);
  const suggested = sel.length ? plRoastOf(sel) : 3;
  ...
  const setPct = (id, v) => {
    setSel(prev => {
      ...
      me.pct = v;
      return plEnforceMins(next, id, batchG);
    });
  };
  const minRoom = 100 - sel.reduce((a,s)=>a+minPct(s.id),0);
  const minsFit = !sel.length || plMinsFit(sel, batchG);
  const add = (id) => {
    if (sel.length >= 4 || sel.some(s=>s.id===id) || minPct(id) > minRoom) return;
    ...
    setSel(plEnforceMins(next, id, batchG));
  };
  const remove = (id) => {
    setSel(prev => {
      ...
      if (next.length) {
        ...
        return plEnforceMins(next, null, batchG);
      }
      return next;
    });
  };
  const track = (id) => (e) => {
    ...
    const apply = (cx) => { const r = el.getBoundingClientRect(); setPct(id, ((cx - r.left)/r.width)*100); };
    apply(e.clientX);
  ...
  const rest = PL_GREEN.filter(c=>!sel.some(s=>s.id===c.id));

  const blocks = {};

  blocks.ratios = (
      <div key="ratios">
        <RuleHead label="Your coffees" right={<span style={{...mono,fontSize:11.5,color:"var(--ink-subtle)"}}>{sel.length} of 4 · {batchLabel || (Math.round(batchG).toLocaleString()+" g")}</span>}/>
        {sel.length === 0 && (
          <div style={{padding:"18px 0 0"}}>
            ...
            <div style={{marginTop:14}}><Btn size="sm" variant="secondary" iconLeft={<I name="plus" size={14} stroke={2.2}/>} onClick={()=>setSel([{id:"cerrado",pct:60},{id:"sierra",pct:40}])}>Start from 60 / 40 anchors</Btn></div>
          </div>
        )}
        {sel.map(s=>{
          const c = plGreen(s.id), col = plRampColor(c.roast);
          return (
            <div key={s.id} style={{padding:"14px 0 12px",borderBottom:"1px solid var(--hairline)"}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                ...
                <span style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",gap:3}}>
                  ...
                  <span style={{fontFamily:"var(--font-sans)",fontSize:12,color:"var(--ink-subtle)"}}>{c.origin} · {plMoney(retail ? plRetailOf(c) : c.price)}/lb{plMinG(c) > 0 && <> · <span style={{...mono,color:s.pct<=minPct(c.id)?"var(--brand)":"var(--ink-subtle)"}}>min {plMinG(c)} g ({minPct(c.id)}%)</span></>}</span>
                </span>
                <span style={{display:"flex",alignItems:"center",gap:2,flexShrink:0}}>
                  <Stepper glyph="−" side="l" onClick={()=>setPct(s.id, s.pct-5)}/>
                  <span style={{minWidth:72,textAlign:"center",display:"flex",flexDirection:"column",lineHeight:1.15}}>
                    ...
                    <span style={{...mono,fontSize:10.5,color:"var(--ink-subtle)"}}>{plGramsOf(s.pct, batchG).toLocaleString()} g</span>
                  </span>
                  <Stepper glyph="+" side="r" onClick={()=>setPct(s.id, s.pct+5)}/>
                  <button onClick={()=>remove(s.id)} aria-label="Remove" style={{width:26,height:26,marginLeft:6,display:"flex",alignItems:"center",justifyContent:"center",border:"none",background:"transparent",color:"var(--ink-subtle)",cursor:"pointer",fontSize:13,lineHeight:1}}>✕</button>
                </span>
              </div>
              <div onPointerDown={track(s.id)} style={{position:"relative",height:30,marginTop:4,display:"flex",alignItems:"center",cursor:"ew-resize",touchAction:"none"}}>
                ...
                {minPct(c.id) > 0 && <span title={`Minimum ${plMinG(c)} g`} style={{position:"absolute",left:minPct(c.id)+"%",width:1,height:12,background:"var(--hairline-strong)"}}/>}
        ...
        {sel.length > 0 && !minsFit && (
          <div style={{display:"flex",alignItems:"flex-start",gap:9,marginTop:12,padding:"11px 13px",borderRadius:"var(--r-md)",background:"var(--brand-soft)",border:"1px solid var(--brand)"}}>
            ...
            <span style={{fontFamily:"var(--font-sans)",fontSize:12.5,lineHeight:1.5,color:"var(--ink)"}}>This batch is {Math.round(batchG).toLocaleString()} g. {sel.length === 1 ? "That's under this coffee's minimum" : "These coffees need " + sel.reduce((a,s)=>a+plMinG(s.id),0).toLocaleString() + " g between them"} — order a bigger size, more bags, or {sel.length === 1 ? "pick another coffee" : "take one out"}.</span>
          </div>
        )}
        {sel.length > 0 && (
  ...
  blocks.roast = sel.length > 0 && (
        <div key="roast">
          <RuleHead label="Roast level" right={roast!=null
            ? <button onClick={()=>setRoast(null)} style={{padding:0,border:"none",background:"transparent",cursor:"pointer",fontFamily:"var(--font-sans)",fontSize:12,color:"var(--brand)"}}>Back to suggested · {plRoastName(suggested)}</button>
          ...
          <div style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap",padding:"14px 0 12px"}}>
            <span style={{display:"flex",height:12,width:170,borderRadius:2,overflow:"hidden",border:"1px solid var(--hairline)"}}>
              {[1,2,3,4,5].map(n=><span key={n} style={{flex:1,background:plRamp5(n),opacity: Math.round(eff)===n?1:.28}}/>)}
            </span>
            <span style={{...mono,fontSize:11.5,color:"var(--ink-subtle)"}}>Agtron {[75,75,67,58,50,42][Math.round(eff)]} · drop {[396,396,407,417,427,436][Math.round(eff)]}°F</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(104px,1fr))",gap:6}}>
            {[1,2,3,4,5].map(n=>{
              const on = Math.round(eff)===n;
              return (
                <button key={n} onClick={()=>setRoast(n)} style={{display:"flex",flexDirection:"column",alignItems:"flex-start",gap:8,padding:"9px 9px 10px",cursor:"pointer",textAlign:"left",
                  border:`1px solid ${on?"var(--brand)":"var(--hairline-strong)"}`,background:on?"var(--brand-soft)":"var(--surface)",borderRadius:"var(--r-sm)"}}>
                  <span style={{width:"100%",height:6,borderRadius:2,background:plRamp5(n)}}/>
                  <span style={{...over,fontSize:9.5,color:on?"var(--brand-hover)":"var(--ink)"}}>{plRoastName(n)}</span>
                  <span style={{fontFamily:"var(--font-sans)",fontSize:11.5,lineHeight:1.3,color:"var(--ink-subtle)"}}>{["","Tea, acidity forward","Sweet, fruit intact","Balanced, caramel","Cocoa, low acid","Smoke, syrupy"][n]}</span>
                </button>
              );
            })}
          </div>
  ...
  blocks.add = (
      <div key="add">
        ...
        {rest.map(c=>{
          const out = c.avail === 0, full = sel.length >= 4, noRoom = minPct(c.id) > minRoom || !plMinsFit([...sel, {id:c.id}], batchG);
          return (
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,padding:"11px 0",opacity:out?.5:1}}>
              <div style={{width:40,height:40,flexShrink:0}}><image-slot id={`green-${c.id}`} shape="rounded" radius="5" placeholder=" "></image-slot></div>
              ...
              <span style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",gap:2}}>
                ...
                <span style={{fontFamily:"var(--font-sans)",fontSize:12,color:"var(--ink-subtle)"}}>{c.origin} · {c.process} · <span style={mono}>{c.avail ? (retail ? "in stock" : c.avail.toLocaleString()+" lb green") : "out of stock"}</span>{plMinG(c) > 0 && <> · <span style={{...mono,color:noRoom?"var(--danger)":"var(--ink-subtle)"}}>min {plMinG(c)} g{noRoom && !out && !full ? " — won't fit this batch" : ""}</span></>}</span>
              </span>
              <button className="bb-add" onClick={()=>add(c.id)} disabled={out||full||noRoom} style={{transition:"background var(--dur) var(--ease), border-color var(--dur) var(--ease), color var(--dur) var(--ease), transform 90ms var(--ease)",height:32,padding:"0 12px",gap:6,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",whiteSpace:"nowrap",fontFamily:"var(--font-sans)",fontWeight:600,
                border:"1px solid var(--hairline-strong)",background:"var(--surface)",borderRadius:"var(--r-sm)",color:"var(--ink)",cursor:(out||full||noRoom)?"not-allowed":"pointer",opacity:(out||full||noRoom)?.4:1,fontSize:12.5,lineHeight:1}}><span aria-hidden="true" style={{...mono,fontSize:14}}>+</span>Add to blend</button>
            </div>
            {window.CoffeeReviews && <div style={{paddingLeft:52}}><CoffeeReviews coffee={c}/></div>}
            </div>
          );
        })}
      </div>
  ...
  return <div style={{display:"flex",flexDirection:"column",gap}}>{sections.map(s=>blocks[s]||null)}</div>;
};

const plRamp5 = (n) => `var(--roast-${n})`;

const RuleHead = ({ label, right }) => (
...
const Stepper = ({ glyph, side, onClick }) => (
  <button onClick={onClick} aria-label={glyph==="+"?"Increase":"Decrease"} style={{width:26,height:26,display:"flex",alignItems:"center",justifyContent:"center",
    ...
    borderRadius: side==="l" ? "var(--r-sm) 0 0 var(--r-sm)" : "0 var(--r-sm) var(--r-sm) 0"}}>{glyph}</button>
);

// ---- the cup: live radar of weighted cupping scores ----
const TastingWheel = ({ vals, roast, empty, title="The cup", note }) => {
  const g = plGeom(vals);
  const rgb = plRampColor(roast);
  const soft = (a) => rgb.replace("rgb(","rgba(").replace(")",`,${a})`);
  const top = PL_AX.map(a=>({label:a.l, v:vals[a.k]||0})).sort((x,y)=>y.v-x.v).slice(0,5);
  return (
    <div style={{background:"var(--surface)",border:"1px solid var(--hairline)",borderRadius:"var(--r-lg)",padding:"20px 22px 22px"}}>
      ...
      <div style={{position:"relative",width:"100%",maxWidth:400,margin:"0 auto",aspectRatio:"1 / 1"}}>
        ...
        {!empty && g.axes.map((a,i)=>(
          <div key={i} style={{position:"absolute",display:"flex",flexDirection:"column",gap:1,whiteSpace:"nowrap",left:a.lx+"%",top:a.ly+"%",transform:a.tf,alignItems:a.al,opacity:a.op}}>
...
Object.assign(window, { BlendRatios, TastingWheel, RuleHead, Stepper, plRamp5 });
