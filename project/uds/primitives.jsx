...
const Btn = ({ variant="primary", size="md", children, onClick, icon, style }) => {
  ...
  const outlined = variant==="secondary" || variant==="outline";
  const h = { sm:32, md:38, lg:46 }[size];
  const px = { sm:14, md:18, lg:24 }[size];
  const fs = { sm:12.5, md:13.5, lg:15 }[size];
  return (
    <button onClick={onClick} className="co-btn" style={{
      display:"inline-flex", alignItems:"center", gap:8, height:h, padding:`0 ${px}px`, borderRadius:10,
      border:outlined?"1px solid var(--color-border)":"none", background:bg, color:fg, fontFamily:"var(--font-body)",
      fontWeight:600, fontSize:fs, letterSpacing:"0",  cursor:"pointer",
      boxShadow:"none", transition:"transform .12s var(--ease-snap), background .12s var(--ease-snap)", whiteSpace:"nowrap", ...style,
...
const Pill = ({ variant="tomato", children, style }) => {
  ...
  return <span style={{
    ...
    border:"1px solid var(--color-border)",background:m.bg,color:m.fg,
    fontWeight:600,fontSize:11,letterSpacing:".01em",...style,
  }}>{children}</span>;
};

const Panel = ({ title, actions, children, noPadding, style }) => (
  <div style={{background:"var(--color-surface)",border:"1px solid var(--color-border)",borderRadius:16,boxShadow:"var(--shadow-sm)",overflow:"hidden",...style}}>
    {title && <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",borderBottom:"1px solid var(--color-border)",background:"var(--color-surface)"}}>
      <div style={{fontFamily:"var(--font-display)",fontSize:18,lineHeight:1}}>{title}</div>
      {actions}
    </div>}
...
const Stat = ({ label, value, delta, accent="tomato" }) => (
  <div style={{background:"var(--color-surface)",border:"1px solid var(--color-border)",borderRadius:16,padding:16,boxShadow:"var(--shadow-sm)"}}>
    <div style={{fontSize:10.5,fontWeight:800,letterSpacing:".12em",color:"var(--fg2)"}}>{label}</div>
    <div style={{fontFamily:"var(--font-display)",fontSize:42,lineHeight:1,marginTop:6,color:"var(--color-espresso)"}}>{value}</div>
    {delta && <div style={{display:"inline-flex",alignItems:"center",gap:4,marginTop:8,fontSize:11,fontWeight:800,letterSpacing:".08em",color:delta.startsWith("+")?"var(--color-matcha)":"var(--color-tomato)"}}>{delta}</div>}
  </div>
);
