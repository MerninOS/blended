...
const BookingDrawer = ({ booking, onClose }) => {
  ...
  return (
    <>
      <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(28,15,5,0.45)",zIndex:400,animation:"co-fade .15s ease-out"}}/>
      <div style={{
        position:"fixed",top:0,right:0,bottom:0,width:"min(560px, 100vw)",
        background:"var(--color-surface)",borderLeft:"1px solid var(--color-border)",
        zIndex:401,boxShadow:"-12px 0 40px rgba(28,15,5,0.10)",
        display:"flex",flexDirection:"column",animation:"co-slide .22s var(--ease-snap)",
        overflowY:"auto",
      }}>
        {/* Header */}
        <div style={{position:"sticky",top:0,background:`var(--color-${meta.color})`,color:`var(--color-${meta.fg})`,borderBottom:"1px solid var(--color-border)",padding:"18px 22px",display:"flex",alignItems:"flex-start",gap:14,zIndex:1}}>
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
              <span style={{fontSize:10,fontWeight:900,letterSpacing:".14em",padding:"3px 8px",background:"rgba(28,15,5,0.18)",borderRadius:6}}>{meta.label} · #{booking.id.toUpperCase()}</span>
              {booking.recurring && <span style={{display:"inline-flex",alignItems:"center",gap:4,fontSize:10,fontWeight:800,letterSpacing:".1em",padding:"3px 8px",background:"rgba(28,15,5,0.18)",borderRadius:6}}><I2 name="repeat" size={11}/> Recurring weekly</span>}
              {booking.status==="inprogress" && <span style={{display:"inline-flex",alignItems:"center",gap:5,fontSize:10,fontWeight:800,letterSpacing:".1em",padding:"3px 8px",background:"var(--color-rail)",color:"var(--color-cream)",borderRadius:6}}><Dot color="sun" pulse size={7}/> On floor</span>}
            </div>
            <h2 style={{fontFamily:"var(--font-display)",fontSize:34,lineHeight:.95,margin:0}}>{cust.name}</h2>
            <div style={{fontSize:13,marginTop:6,fontWeight:700,opacity:0.85}}>{booking.lot} · {booking.lbs} lb · {booking.profile}</div>
          </div>
          <button onClick={onClose} style={{width:36,height:36,border:"1px solid var(--color-border)",borderRadius:11999,background:"var(--color-surface)",color:"var(--color-espresso)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"var(--shadow-sm)"}}><I2 name="x" size={16}/></button>
        </div>

        <div style={{padding:22,display:"flex",flexDirection:"column",gap:16}}>
          {/* Schedule strip */}
          <div style={{padding:14,border:"1px solid var(--color-border)",borderRadius:16,background:"var(--color-surface)",display:"flex",alignItems:"center",gap:14,boxShadow:"var(--shadow-sm)"}}>
            <I2 name="cal" size={22}/>
            <div style={{flex:1}}>
              <div style={{fontSize:10,fontWeight:800,letterSpacing:".12em",color:"var(--fg2)"}}>Saturday · Apr 25, 2026</div>
              <div style={{fontFamily:"var(--font-display)",fontSize:24,lineHeight:1,marginTop:4}}>{fmtTimeLong(booking.start)} – {fmtTimeLong(booking.end)}</div>
          ...
          {/* Customer */}
          <div style={{padding:14,border:"1px solid var(--color-border)",borderRadius:16,background:"var(--color-surface)",display:"flex",alignItems:"center",gap:12,boxShadow:"var(--shadow-sm)"}}>
          ...
          <Panel title="Pricing" actions={<Pill variant={booking.paid?"matcha":"tomato"}>{booking.paid?"Paid":"Unpaid"}</Pill>}>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              ...
              <div style={{display:"flex",alignItems:"center"}}>
                <div style={{flex:1,fontFamily:"var(--font-display)",fontSize:24}}>Total</div>
...
const InfoTile = ({icon,label,value,hint}) => (
  <div style={{padding:12,border:"1px solid var(--color-border)",borderRadius:12,background:"var(--color-surface)"}}>
    <div style={{display:"flex",alignItems:"center",gap:6,fontSize:10,fontWeight:800,letterSpacing:".11em",color:"var(--fg2)"}}>
      <I2 name={icon} size={12}/>{label}
    </div>
    <div style={{fontFamily:"var(--font-display)",fontSize:18,lineHeight:1,marginTop:6}}>{value}</div>
