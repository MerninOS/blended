            ...
            {onFloor.map(b=>{
              ...
              return (
                <button key={b.id} onClick={()=>openBooking(b)} style={floorCardStyle()}
                  onMouseOver={e=>{e.currentTarget.style.transform="translate(-2px,-2px)";e.currentTarget.style.boxShadow="var(--shadow-md)"}}
                  onMouseOut={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="var(--shadow-md)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <div style={{width:48,height:48,background:"var(--color-tomato)",border:"1px solid var(--color-border)",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--color-cream)",position:"relative"}}>
                      <I name="flame" size={22}/>
                      <span style={{position:"absolute",top:-6,right:-6,width:14,height:14,background:"var(--color-sun)",border:"1px solid var(--color-border)",borderRadius:11999,animation:"co-pulse 1.5s ease-in-out infinite"}}/>
                    </div>
                    <div style={{flex:1,minWidth:0,textAlign:"left"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                        <TypeChip type={b.type}/>
                        <span style={{fontSize:11,color:"var(--fg2)",fontWeight:700,letterSpacing:".06em"}}>{res.name}</span>
                      </div>
                      <div style={{fontFamily:"var(--font-display)",fontSize:22,lineHeight:1}}>{cust.name}</div>
                    ...
                    <div style={{textAlign:"right",flexShrink:0}}>
                      ...
                      <div style={{fontSize:9.5,fontWeight:800,letterSpacing:".1em",color:"var(--fg2)"}}>elapsed</div>
                    </div>
                  </div>
                  <div style={{height:8,background:"var(--color-fog)",border:"1px solid var(--color-border)",borderRadius:11999,marginTop:10,overflow:"hidden",position:"relative"}}>
                    <div style={{position:"absolute",inset:0,width:pct+"%",background:"var(--color-tomato)",borderRight:"1px solid var(--color-border)"}}/>
                  </div>
                </button>
              );
            })}
            ...
            {upNext.map(b=>{
              ...
              return (
                <button key={b.id} onClick={()=>openBooking(b)} style={upNextStyle()}
                  onMouseOver={e=>{e.currentTarget.style.transform="translate(-1.5px,-1.5px)";e.currentTarget.style.boxShadow="var(--shadow-md)"}}
                  onMouseOut={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="var(--shadow-md)"}}>
                  <div style={{textAlign:"center",flexShrink:0,width:54}}>
                    ...
                    <div style={{fontSize:9,fontWeight:800,letterSpacing:".1em",color:"var(--fg2)"}}>{fmtTime(b.start).slice(-1)==="p"?"PM":"AM"}</div>
                  </div>
                  ...
                  <div style={{textAlign:"right",fontSize:10,fontWeight:800,letterSpacing:".08em",color:"var(--color-tomato)"}}>
              ...
              return (
                <div key={k} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
                  <div style={{position:"relative",width:"100%",height:110,display:"flex",alignItems:"flex-end"}}>
                    <div style={{width:"100%",height:pct*1.05+"%",background:`var(--color-${meta.color})`,border:"1px solid var(--color-border)",borderRadius:"8px 8px 0 0",boxShadow:"var(--shadow-sm)",position:"relative"}}>
                  ...
                  <div style={{fontSize:10,fontWeight:800,letterSpacing:".1em"}}>{meta.label}</div>
                </div>
              );
            })}
          </div>
...
const AlertRow = ({icon,color,title,sub,cta,onClick}) => (
  <div style={{padding:12,border:"1px solid var(--color-border)",borderRadius:12,background:"var(--color-surface)",display:"flex",alignItems:"center",gap:12}}>
    <div style={{width:36,height:36,background:`var(--color-${color})`,border:"1px solid var(--color-border)",borderRadius:11999,display:"flex",alignItems:"center",justifyContent:"center",color:(color==="sun"||color==="sky")?"var(--color-espresso)":"var(--color-cream)",flexShrink:0}}>
...
const floorCardStyle = () => ({
  padding:14,border:"1px solid var(--color-border)",borderRadius:16,
...
const upNextStyle = () => ({
  padding:"10px 12px",border:"1px solid var(--color-border)",borderRadius:12,
  background:"var(--color-surface)",boxShadow:"var(--shadow-sm)",
  cursor:"pointer",transition:"all .12s var(--ease-snap)",fontFamily:"var(--font-body)",
...
const SignoffBanner = ({sessions, openBooking}) => {
  const NOW_HR = 10.3;
  return (
    <div style={{padding:"14px 18px",background:"var(--color-sun)",color:"var(--color-espresso)",border:"1px solid var(--color-border)",borderRadius:16,boxShadow:"var(--shadow-sm)",display:"flex",alignItems:"stretch",gap:16}}>
      <div style={{width:48,height:48,background:"var(--color-espresso)",border:"1px solid var(--color-border)",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--color-sun)",flexShrink:0,alignSelf:"center"}}>
        <I2 name="alert" size={22}/>
      </div>
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:3}}>
          <div style={{fontFamily:"var(--font-display)",fontSize:20,lineHeight:1}}>Sessions need sign-off</div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {sessions.map(b=>{
            ...
            return (
              <div key={b.id} style={{padding:"9px 12px",background:"var(--color-surface)",border:"1px solid var(--color-border)",borderRadius:12,display:"flex",alignItems:"center",gap:12}}>
