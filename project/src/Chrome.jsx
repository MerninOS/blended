...
const Sidebar = ({ section, setSection }) => {
  ...
  return (
    <aside style={{width:236,background:"var(--color-espresso)",color:"var(--color-cream)",padding:"18px 12px",display:"flex",flexDirection:"column",gap:3,flexShrink:0,minHeight:"100vh",position:"sticky",top:0,alignSelf:"flex-start"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"4px 8px 16px",borderBottom:"2px solid var(--color-roast)",marginBottom:12}}>
        <div style={{width:36,height:36,background:"var(--color-tomato)",border:"2.5px solid var(--color-cream)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--color-cream)",fontFamily:"var(--font-display)",fontSize:22,lineHeight:1}}>C</div>
        <div>
        ...
        return (
          ...
          onMouseOut={e=>{  if(!active) e.currentTarget.style.background="transparent"; }}>
            ...
            {n.count!=null && <span style={{
              ...
              border:"1.5px solid var(--color-espresso)",fontWeight:800
            }}>{n.count}</span>}
          </button>
        );
      })}
      <div style={{flex:1}}/>
      <div style={{padding:12,background:"var(--color-roast)",borderRadius:12,border:"2px solid var(--color-tomato)",fontSize:11,lineHeight:1.4,marginTop:12}}>
...
const TopBar = ({ title, subtitle, breadcrumbs, actions }) => (
  <div style={{background:"var(--color-cream)",padding:"18px 24px 16px",borderBottom:"3px solid var(--color-espresso)",display:"flex",alignItems:"center",justifyContent:"space-between",gap:20,position:"sticky",top:0,zIndex:5}}>
