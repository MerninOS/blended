...
const Starburst = ({ label, color="sun", size=80, style }) => {
  ...
  return (
    <div style={{position:"relative",display:"inline-flex",width:size,height:size,alignItems:"center",justifyContent:"center",...style}}>
      ...
      <span style={{position:"relative",zIndex:2,color:m.fg,fontFamily:"var(--font-body)",fontWeight:800,fontSize:Math.max(10,Math.round(size*0.14)),letterSpacing:".04em",textTransform:"uppercase",lineHeight:1.05,textAlign:"center",padding:"0 14%"}}>{label}</span>
    </div>
  );
};
