    ...
    <div style={{padding:"16px 24px 32px",display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          {Object.entries(BOOKING_TYPES).filter(([k])=>k!=="storage").map(([k,m])=>(
            <span key={k} style={{display:"inline-flex",alignItems:"center",gap:6,fontSize:11,fontWeight:700,letterSpacing:".05em"}}>
              <span style={{width:14,height:14,background:`var(--color-${m.color})`,border:"2px solid var(--color-espresso)",borderRadius:4}}/>
              {m.label}
            </span>
          ))}
      ...
      <Panel noPadding>
        <div style={{display:"flex",fontFamily:"var(--font-body)",position:"relative"}}>
          ...
          <div style={{flexShrink:0,width:LABEL_W,borderRight:"3px solid var(--color-espresso)",background:"var(--color-cream)"}}>
            {/* header spacer */}
            <div style={{height:36,borderBottom:"2px solid var(--color-espresso)",display:"flex",alignItems:"center",padding:"0 14px",fontSize:10,fontWeight:800,letterSpacing:".12em",textTransform:"uppercase",color:"var(--fg2)",background:"var(--color-chalk)"}}>Resource</div>
            {RESOURCES.map(r=>(
              <div key={r.id} style={{height:ROW_H,borderBottom:"1px solid var(--color-fog)",padding:"0 14px",display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:32,height:32,background:`var(--color-${r.color})`,border:"2.5px solid var(--color-espresso)",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",color: (r.color==="sun"||r.color==="sky"||r.color==="cream") ? "var(--color-espresso)" : "var(--color-cream)",flexShrink:0}}>
          ...
          <div style={{flex:1,overflowX:"auto",position:"relative"}}>
            <div style={{position:"relative",width:totalSlots*SLOT_W,minWidth:"100%"}}>
              {/* Header: hours */}
              <div style={{display:"flex",height:36,borderBottom:"2px solid var(--color-espresso)",background:"var(--color-chalk)"}}>
                    ...
                    return (
                      <button key={b.id} onClick={()=>openBooking(b)} style={blockStyle(b,r)}
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:6,minWidth:0}}>
                          ...
                          {!b.paid && b.status!=="completed" && <span title="unpaid" style={{fontSize:9,fontWeight:900,letterSpacing:".08em",textTransform:"uppercase",background:"var(--color-espresso)",color:"var(--color-cream)",padding:"1px 4px",borderRadius:3}}>$</span>}
                        </div>
                        <div style={{minWidth:0}}>
              ...
              {showNow && (
                <div style={{
                  ...
                  <div style={{position:"absolute",top:-6,left:-7,width:16,height:16,background:"var(--color-tomato)",border:"2.5px solid var(--color-espresso)",borderRadius:9999}}/>
                  <div style={{position:"absolute",top:-22,left:6,fontSize:10,fontWeight:800,letterSpacing:".12em",textTransform:"uppercase",color:"var(--color-tomato)",fontFamily:"var(--font-body)",background:"var(--color-cream)",padding:"1px 4px",border:"1.5px solid var(--color-tomato)",borderRadius:4,whiteSpace:"nowrap"}}>Now {fmtTime(NOW_HR)}</div>
                </div>
              )}
            </div>
          </div>
...
const navArrow = (disabled) => ({
  width:36,height:36,border:"2.5px solid var(--color-espresso)",borderRadius:9999,
