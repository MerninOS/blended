...
const TypeChip = ({ type, size="sm" }) => {
  ...
  return <span style={{
    ...
    borderRadius:9999,border:"2px solid var(--color-espresso)",
    background:`var(--color-${meta.color})`,color:`var(--color-${meta.fg})`,
    fontWeight:800,fontSize:fs,letterSpacing:".1em",textTransform:"uppercase",whiteSpace:"nowrap"
  }}>{meta.label}</span>;
};
...
const Dot = ({ color="matcha", pulse=false, size=8 }) => (
  <span style={{
    ...
    background:`var(--color-${color})`,border:"1.5px solid var(--color-espresso)",
