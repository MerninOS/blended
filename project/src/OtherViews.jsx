    ...
    <div style={{padding:"16px 24px 32px",display:"flex",flexDirection:"column",gap:14}}>
      ...
      <Panel title="Roasters" actions={
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
          <thead><tr style={{borderBottom:"2px solid var(--color-espresso)"}}>
  ...
  return (
    <div style={{padding:"16px 24px 32px",display:"flex",flexDirection:"column",gap:14}}>
      ...
      <Panel title="Shelf Map" actions={<div style={{display:"flex",gap:10,fontSize:11,fontWeight:700}}>
        <span style={{display:"inline-flex",alignItems:"center",gap:5}}><span style={{width:12,height:12,background:"var(--color-matcha)",border:"2px solid var(--color-espresso)",borderRadius:3}}/>Active</span>
        <span style={{display:"inline-flex",alignItems:"center",gap:5}}><span style={{width:12,height:12,background:"var(--color-tomato)",border:"2px solid var(--color-espresso)",borderRadius:3}}/>Past due</span>
        <span style={{display:"inline-flex",alignItems:"center",gap:5}}><span style={{width:12,height:12,background:"var(--color-cream)",border:"2px solid var(--color-espresso)",borderRadius:3}}/>Open</span>
      </div>}>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {rows.map(r=>(
            <div key={r} style={{display:"flex",alignItems:"stretch",gap:10}}>
              <div style={{width:54,display:"flex",alignItems:"center",justifyContent:"center",background:"var(--color-espresso)",color:"var(--color-cream)",border:"2.5px solid var(--color-espresso)",borderRadius:10,fontFamily:"var(--font-display)",fontSize:30,lineHeight:1}}>{r}</div>
              <div style={{flex:1,display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(180px, 1fr))",gap:8}}>
                {SHELVES.filter(s=>s.row===r).map(s=>{
                  ...
                  return (
                    <div key={s.id} style={{padding:10,border:"2.5px solid var(--color-espresso)",borderRadius:10,background:bg,boxShadow:"2px 2px 0 var(--color-espresso)",display:"flex",flexDirection:"column",gap:6,minHeight:88}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                        <span style={{fontFamily:"var(--font-mono)",fontWeight:800,fontSize:11,padding:"2px 6px",background:"var(--color-espresso)",color:"var(--color-cream)",borderRadius:5}}>{s.row}-{s.num}</span>
            ...
            {active.map(s=>{
              ...
              return (
                <tr key={s.id} style={{borderBottom:"1px dashed var(--color-fog)"}}>
                  <td style={{padding:"9px 10px"}}><span style={{fontFamily:"var(--font-mono)",fontWeight:800,fontSize:12,padding:"3px 8px",background:"var(--color-espresso)",color:"var(--color-cream)",borderRadius:5}}>{s.row}-{s.num}</span></td>
  ...
  <div style={{padding:"16px 24px 32px",display:"flex",flexDirection:"column",gap:14}}>
    <Panel title="What We Offer" actions={<Btn size="sm" variant="primary" icon={<I name="plus" size={14}/>}>Add Service</Btn>}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(310px, 1fr))",gap:12}}>
        {SERVICES.map(s=>(
          <div key={s.id} style={{padding:14,border:"3px solid var(--color-espresso)",borderRadius:14,background:"var(--color-chalk)",boxShadow:"3px 3px 0 var(--color-espresso)",display:"flex",flexDirection:"column",gap:8}}>
...
const QBLogo = ({size=14}) => (
  <span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:size+4,height:size+4,borderRadius:9999,background:"#2CA01C",color:"#fff",fontFamily:"var(--font-display)",fontSize:size-2,fontWeight:900,letterSpacing:"-.03em"}}>qb</span>
);
...
const IntegrationsBar = ({draftCount}) => (
  <div style={{padding:"12px 16px",background:"var(--color-espresso)",color:"var(--color-cream)",border:"3px solid var(--color-espresso)",borderRadius:14,boxShadow:"3px 3px 0 var(--color-tomato)",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",flex:1,minWidth:0}}>
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",background:"rgba(255,255,255,.08)",borderRadius:8,whiteSpace:"nowrap"}}>
        <QBLogo size={14}/>
        ...
        <span style={{width:8,height:8,borderRadius:9999,background:"var(--color-matcha)",marginLeft:4,flexShrink:0}}/>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",background:"rgba(255,255,255,.08)",borderRadius:8,whiteSpace:"nowrap"}}>
        <StripeLogo size={14}/>
        <div style={{lineHeight:1.15,whiteSpace:"nowrap"}}>
    ...
    {draftCount>0 && (
      <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        ...
        <button style={{padding:"8px 14px",border:"2.5px solid var(--color-cream)",background:"var(--color-cream)",color:"var(--color-espresso)",borderRadius:9,fontWeight:800,fontSize:12,fontFamily:"var(--font-body)",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:6,letterSpacing:".02em",whiteSpace:"nowrap"}}>
